...
export class EditComponent implements OnInit, OnDestroy{
  ...
  constructor(public tl: TablayoutService, public sconf: SconfService) {
    this.cud = this.tl.genResful('StockOrders');
    this.xl = this.tl.genResful('StockOrderEditQuery');
    tl.editobj$.pipe(takeUntil(this.ngUnsubscribe)).subscribe(async o => {
      

      this.refobj = { ...o };
      this.env.luckedit = this.refobj.ProcessDate  ? true:false;
      this.locstockunits$.next(sconf.xstockunitsAdmin$.getValue().filter(u => u.Id !== this.refobj.OrgStockUnitId));
  ...
  async stockOrderchange(d: string) {
    ...
    const o = await this.tl.sharehttp.get<IIdCount[]>(this.tl.sharebaseUrl + 'api/Query/StockOrderBarcodes/' + d).pipe(take(1)).toPromise();
    ...
    this.xl.preQuery({ StockOrderId: d, OrgStockUnitId: this.refobj.OrgStockUnitId, ProcessDate: this.refobj.ProcessDate });
  ...
  async doMod() {
   ...
   // const fbs = await this.tl.sharehttp.post<IStockOrderSub[]>(`${this.tl.sharebaseUrl}api/Query/AskStockOrderFormateBarcodes`, { StockOrderId: this.refobj.StockOrderId, FormateNumbrs: FormateNumbrs, BarcodeIds: BarcodeIds }).toPromise();
    ...
    console.log(o);
    const j = await this.cud.del(o.StockOrderId);
    if (j) {
