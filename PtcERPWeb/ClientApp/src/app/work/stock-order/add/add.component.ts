...
export class AddComponent implements OnInit,OnDestroy {
  ...
  constructor(public tl: TablayoutService, public sconf: SconfService, private  alts: AlertService) {
    ...
    this.xl = this.tl.genResful('StockOrderAddQuery');
    tl.preAdd$.pipe(takeUntil(this.ngUnsubscribe)).subscribe(async o => {
      ...
      if (!allprocess) {
        ...
        if (this.tl.actionqAddurl != 'StockOrdersAddQuery') {
          this.tl.initAdd('StockOrdersAddQuery')
        }
        this.refobj = o;
        this.preView = false;
  ...
  async dvchange(d) {
    if (d) {
      ...
      if (this.refobj.StockUnitId > 0) {
        const orgs = this.sconf.xstockunitsAdmin$.getValue();
